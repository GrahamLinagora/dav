  ...
  if (!filter.children || !filter.children.length) {
    if (filter.value) {
      return new Handlebars.SafeString(
        format('<c:%s %s>%s</c:%s>', filter.type,
          formatAttrs(filter.attrs), filter.value, filter.type)
      );
    }
    else {
      return new Handlebars.SafeString(
...
function propHelper(prop) {
	...
	if (prop.value) {
		if (prop.attrs) {
      formattedString = format('<%s:%s %s>%s</%s:%s>', nsprefix, prop.name,
				formatAttrs(prop.attrs), prop.value, nsprefix, prop.name);
    }
    else {
		...
		if (prop.attrs) {
      formattedString = format('<%s:%s %s/>', nsprefix,
        prop.name, formatAttrs(prop.attrs));
    }
    else {
